...
//	move and stop
	...
	public class getButtonListener3 implements ActionListener{
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			try{
				exist = new File("draw.dat"+sc.nextInt());
				if(exist.exists())
					throw new IllegalArgumentException("There's exists same name file!");
				fos = new FileOutputStream(exist);
				...
				for(BasicStroke next : f.stroke)
					oos.writeFloat(next.getLineWidth());
	...
	public class getButtonListener4 implements ActionListener{
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			try{
				...
				for(int i=0; i<f.i; i++)
					f.stroke.add(new BasicStroke(ois.readFloat()));
